...
def parse_args(
    ...
    asr_defaults: AsrSettings,
    intent_defaults: IntentSettings,
) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Free6GC computing Sandbox mock service"
    )
    ...
    parser.add_argument("--asr-host", default=asr_defaults.host)
    parser.add_argument("--asr-port", type=int, default=asr_defaults.port)
    parser.add_argument("--intent-host", default=intent_defaults.host)
    parser.add_argument("--intent-port", type=int, default=intent_defaults.port)
...
async def serve(
    ...
    """Run the four pruned_sandbox listeners inside one mock process."""
    ...
    asr_runner = web.AppRunner(create_asr_app(asr_settings))
    intent_runner = web.AppRunner(create_intent_app(intent_settings))
    await management_runner.setup()
    await user_runner.setup()
    await asr_runner.setup()
    await intent_runner.setup()
    await web.TCPSite(
    ...
    await web.TCPSite(asr_runner, asr_settings.host, asr_settings.port).start()
    await web.TCPSite(
        intent_runner, intent_settings.host, intent_settings.port
    ).start()
    LOGGER.info(
        "Sandbox management=%s:%d user=%s:%d asr=%s:%d intent=%s:%d",
        ...
        asr_settings.host,
        asr_settings.port,
        intent_settings.host,
        intent_settings.port,
    )
...
def main() -> None:
    ...
    asr_defaults = AsrSettings.from_env()
    intent_defaults = IntentSettings.from_env()
    args = parse_args(defaults, asr_defaults, intent_defaults)
    settings = replace(
        defaults,
        host=args.host,
        port=args.user_port,
    ...
    intent_settings = replace(
        intent_defaults,
        host=args.intent_host,
        port=args.intent_port,
    )
    asyncio.run(serve(settings, asr_settings, intent_settings))
