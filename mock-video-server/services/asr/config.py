...
class AsrSettings:
    ...
    def from_env(cls) -> "AsrSettings":
        ...
        download_root = os.getenv("ASR_DOWNLOAD_ROOT", "").strip()
        return cls(
            ...
            # Mock 不加载 Whisper 权重，转写返回固定结果。
            model=os.getenv("ASR_MODEL", "mock-fixed-transcription").strip(),
            download_root=download_root or None,
            device=os.getenv("ASR_DEVICE", "cpu").strip(),
            compute_type=os.getenv("ASR_COMPUTE_TYPE", "fixed").strip(),
