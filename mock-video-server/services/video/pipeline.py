...
class FramePipeline:
    ...
    def __init__(self, settings: VideoSettings, detector: YoloDetector) -> None:
        ...
        self._latest_image = self._placeholder()
    ...
    async def _process_frames(
        ...
        while True:
            ...
            try:
                ...
                annotated = image
                detections = []
                LOGGER.exception("固定结果检测失败，当前帧按原图输出")
            if self._source_id != source_id:
                return
    ...
    def _placeholder(self) -> np.ndarray:
        ...
        canvas[:, :] = (12, 12, 12)
        center_top = self.settings.video_height // 2 - 28
        center_bottom = center_top + 56
        canvas[center_top:center_bottom, 72:self.settings.video_width - 72] = (30, 30, 30)
        stripe_top = self.settings.video_height // 2 - 8
        canvas[stripe_top:stripe_top + 16, 112:self.settings.video_width - 112] = (80, 80, 80)
        return canvas
    ...
    def _fit_output(self, image: np.ndarray) -> np.ndarray:
        ...
        resized = (
            ...
            .reformat(width=resized_width, height=resized_height, format="bgr24")
            .to_ndarray()
        )
