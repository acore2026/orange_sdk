...
// DialWithHeaders dials a proxied connection and includes caller-provided
// headers on the extended CONNECT request. The Capsule-Protocol header is
// always controlled by this package.
func DialWithHeaders(ctx context.Context, conn *http3.ClientConn, template *uritemplate.Template, headers http.Header) (*Conn, *http.Response, error) {
	...
	requestHeaders.Set(http3.CapsuleProtocolHeader, capsuleProtocolHeaderValue)
