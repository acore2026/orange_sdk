...
async def test_a2a_retry_with_same_message_id_is_acknowledged_once(sdk_fixture):
    sdk = sdk_fixture["sdk"]
    server = sdk_fixture["server"]
    runtime = sdk_fixture["runtime"]
    sdk.register_network_message_listener(AckNetworkListener())
    listener = RecordingGroupListener()
    sdk.register_group_message_listener(listener)
    assert await runtime.deliver_group_config(group_payload()) is NetworkMessageAction.ACK
    message = {
        "message_id": "m-retry",
        "group_id": "g1",
        "src_agent_id": PEER_ID,
        "dst_agent_id": "did:example:agent-a",
        "type": "computing_video_session",
        "task_id": "computing:css-001",
        "timestamp": "2026-08-21T09:00:00Z",
        "payload": {"compute_service_session_id": "css-001"},
    }

    await server.a2a(message)
    await server.a2a(message)

    assert listener.messages == [
        ("g1", PEER_ID, {"compute_service_session_id": "css-001"})
    ]
