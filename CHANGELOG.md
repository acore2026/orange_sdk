...
- Sandbox ARM64 镜像、Compose 配置和启动命令分散，目标 N6 主机无法通过一个交付文件
  完成镜像导入、旧容器替换、启动和健康检查。

### 修改方式

- 新增宿主机 `start_sandbox.sh`：校验离线包和镜像、按需导入 ARM64 镜像、创建缺失的
  `compose_n6` 网络、删除运行中或已停止的旧 Sandbox 容器、Compose 强制重建并等待健康。
- 新增可复现的 `package-sandbox.sh`，将启动脚本、Compose、ARM64 镜像及校验文件、完整
  Sandbox 源码和烟测程序打包为 `sandbox.tar.gz`，并生成包内清单和外层 SHA-256。

### 验证内容

- 校验 Shell 语法、ARM64 镜像平台和原始镜像 SHA-256；验证离线包清单、解包结构、启动
  参数帮助及现有容器强制替换路径。

## 2026-09-16 — 宿主机 A/B 启动脚本接管 Compose 编排

### 修改原因

- 原 `start-agent-a.sh` 和 `start-agent-b.sh` 被当作容器入口，目标主机仍需手工执行镜像
  导入、Compose 创建、启动和日志检查，无法用单条脚本命令可靠启动或重启 A/B。
- `force-register` 重启旧容器时若走正常停止流程，旧进程会执行退出注销，违背不注销旧身份
  并将本地状态硬重置到状态1的语义。

### 修改方式

- `start-agent-a.sh` 和 `start-agent-b.sh` 改为宿主机入口，共用 Compose 编排器：自动创建
  默认 `.env`、按需校验并导入 ARM64 镜像、重建对应服务、等待就绪事件并在失败时输出日志。
- 原容器入口改名为 `run-agent-a.sh` 和 `run-agent-b.sh`。`fresh-register` 正常停止并重建旧
  容器；`force-register` 先强制停止并删除旧容器，避免退出注销，再用持久卷执行本地 Reset。
- Python SDK 与 ARM64 镜像升级为 `0.17.9`，README 的正常启动流程缩减为先执行 B 启动脚本，
  再执行 A 启动脚本。

### 验证内容

- 增加宿主机编排回归测试，覆盖 A/B 的 fresh/force 四条路径、强制停止差异、服务重建和
  就绪检查；执行 Python 全量测试、Compose 展开校验及 ARM64 镜像构建、自检和归档校验。
...
- 独立 ASR 调用成功后只在通用状态区和日志中显示结果，后续状态事件可能覆盖文本，用户无法
  在操作区持续查看刚刚识别出的内容。

### 修改方式

- Generic 与 RayNeo App 的语音按钮下方新增“最近转写结果”区域；录音、识别和完成状态均在
  该区域展示，成功后的文字一直保留到下一次识别或算力会话清理。
- Android测试App升级为`0.2.40`、`versionCode=42`。

### 验证内容

- 更新 App 接线测试，并执行两个 Flavor 的单元测试、Lint 和 Debug APK 构建。
...
- SDK虽然已解析C-04的`cause`、`status_revision`和`result`，但媒体接口在会话进入终态时
  只返回`state`，导致端侧诊断无法判断资源激活、Sandbox绑定或参与端确认中的具体故障。
- App诊断过滤器未包含WebRTC日志标签，进入媒体协商后的ICE候选信息无法稳定出现在精简日志中。

### 修改方式

- Android SDK记录每次有效的算力状态更新；终态异常完整携带状态、原因、修订号和服务端结果，
  用户接口签名保持不变。
- App诊断采集加入`AgentSdkWebRtc`，版本升级为`0.2.31`、`versionCode=33`。

### 验证内容

- 新增终态C-04诊断回归测试，并执行Android SDK/App全量测试与debug APK构建。
